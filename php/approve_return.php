...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!remove_borrow_record($user_id, $book_id)) {
        echo json_encode(['success' => false, 'message' => 'Error approving return request']);
        exit();
    }
    ...
    echo json_encode(['success' => true, 'message' => 'Return request approved successfully']);
    exit();
} else {
    // Not a POST request
...
// Function to remove borrow record
function remove_borrow_record($user_id, $book_id) {
    $borrowed_books = [];
    $removed = false;
    ...
    if (($handle = fopen('../data/borrowed_books.csv', 'r')) !== FALSE) {
        ...
        while (($data = fgetcsv($handle)) !== FALSE) {
            if (count($data) >= 5 && $data[0] === $user_id && $data[1] === $book_id && $data[4] === 'return_pending') {
