...
if (($handle = fopen($borrowed_books_path, 'r')) !== FALSE) {
    ...
    while (($data = fgetcsv($handle)) !== FALSE) {
        if (count($data) >= 5 && $data[4] === 'return_pending') {
            $user_id = $data[0];
            $book_id = $data[1];
            $borrow_date = $data[2];
            $due_date = $data[3];
    ...
    // Return return requests as JSON
    header('Content-Type: application/json');
    echo json_encode(['success' => true, 'return_requests' => $return_requests]);
    exit();
} else {
    logError("Failed to open borrowed books CSV file: $borrowed_books_path");
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => 'Error loading return requests data']);
    exit();
}
